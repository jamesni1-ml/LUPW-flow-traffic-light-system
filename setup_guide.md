# Hardware Setup Guide — LUPW Flow Traffic Light System (v4)
...
| Blue LED (5mm, standard) | 1 | ~20mA, ~3.2V forward voltage |
| Green LED (5mm, standard) | 1 | ~20mA, ~2.2V forward voltage |
| 150Ω resistor | 3 | Current limiting for red, amber, green LEDs |
| 56Ω resistor | 1 | Current limiting for blue LED (see calculation below) |
| 2N2222 NPN transistor | 4 | To drive LEDs over 5m cable |
| 1kΩ resistor | 4 | Base resistor for transistors |
| 2-core cable (0.75mm²) | ~24m | 4×5m runs + spare (e.g. speaker wire or bell wire) |
...
|---|---|---|---|
| **RED** | No flow (position ≤ zero_pos) | Red on, others off | GPIO 27 (Pin 13) |
| **AMBER** | Rinse 1 (position ≤ rinse1_pos, ~10 GPM) | Amber on, others off | GPIO 22 (Pin 15) |
| **BLUE** | Rinse 2 (position ≤ rinse2_pos, ~25 GPM) | Blue on, others off | GPIO 23 (Pin 16) |
| **GREEN** | Online (position > rinse2_pos) | Green on, others off | GPIO 17 (Pin 11) |
...
**Circuit at the Pi (for EACH LED — repeat for green, amber, blue, and red):**
...
**Blue LED:**
```
  GPIO 23 (Pin 16) ──[ 1kΩ ]──► 2N2222 Base
                                 Emitter → GND (Pin 25)
                                 Collector → 5m cable → 56Ω → Blue LED → 5m return → GND
...
Vled ≈ 2.0V (red) / 2.1V (amber) / 3.2V (blue) / 2.2V (green)
...
Blue:  R = (5V - 0.7V - 0.2V - 3.2V) / 0.015A =  60Ω → use 56Ω standard
...
```
  ...
  GPIO 23 (Pin 16) ── [  56Ω ] ── Blue LED (+)   ── GND (Pin 25)
...
⚠️ Blue LED uses 56Ω (not 150Ω) because blue LEDs have a higher forward voltage (~3.2V).
...
| Blue LED | GPIO 23 | Pin 16 | 1kΩ → 2N2222 base | 5m 2-core | 56Ω → Blue LED → return |
...
| Ground | — | Pin 9, 14, 20, 25 | Transistor emitter | Via return cable | LED cathode |
...
- That's **4 cables × 5m = 20m total cable** (plus spare)
...
### Calibration Mode (measure position thresholds)
```bash
python3 inference.py --model best.pt --calibrate
# Shows live position ratio — note the values at each flow level
```

### With Live Display (requires monitor/VNC)
```bash
python3 inference.py --model best.pt --zero-pos 0.05 --rinse1-pos 0.22 --rinse2-pos 0.45 --display
```
...
| `--calibrate` | off | Run in calibration mode (prints live position ratio) |
...
| Only 2 or 3 LEDs work | Check the failing transistor's base resistor and GPIO pin connection. Test with `python3 test_gpio.py`. |
...
| Wrong light colour | Check `--zero-pos`, `--rinse1-pos`, `--rinse2-pos` thresholds. Run `--calibrate` to see live position ratios. Run `test_gpio.py` to verify which GPIO controls which LED. |
